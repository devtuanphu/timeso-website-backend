...
async function createCollectionEntry(endpoint, data) {
    var _a;
    try {
        const res = await fetch(`${STRAPI_URL}/api/${endpoint}`, {
            method: "POST",
        ...
        const result = (await res.json());
        const id = (_a = result === null || result === void 0 ? void 0 : result.data) === null || _a === void 0 ? void 0 : _a.id;
        return id ? { id } : null;
    }
    catch (e) {
...
async function seedThanhVien() {
    ...
    const members = [
        {
            ...
            mo_ta: "Hơn 10 năm kinh nghiệm trong lĩnh vực quản lý nhân sự và công nghệ. Từng giữ vị trí quản lý cấp cao tại nhiều công ty công nghệ hàng đầu.",
        ...
        },
        {
            ...
            mo_ta: "Chuyên gia công nghệ với hơn 8 năm kinh nghiệm phát triển phần mềm. Đam mê tạo ra các giải pháp công nghệ thông minh và hiệu quả.",
        ...
        },
        {
            ...
            mo_ta: "Với nền tảng UX/UI và quản lý sản phẩm, anh đã giúp nhiều sản phẩm công nghệ đạt được thành công trên thị trường.",
        ...
        },
        {
            ...
            mo_ta: "Kiến trúc sư phần mềm với kinh nghiệm xây dựng hệ thống quy mô lớn, đảm bảo hiệu suất và độ ổn định cho sản phẩm.",
    ...
    for (const m of members) {
        ...
        const result = await createCollectionEntry("thanh-viens", {
            ...
            chuc_vu: m.chuc_vu,
            mo_ta: m.mo_ta,
            avatar: (_a = avatar === null || avatar === void 0 ? void 0 : avatar.id) !== null && _a !== void 0 ? _a : null,
            thu_tu: m.thu_tu,
        });
        if (result) {
...
async function seedDoiTac() {
    ...
    for (const p of partners) {
        ...
        const result = await createCollectionEntry("doi-tacs", {
            ten: p.ten,
            logo: (_a = logo === null || logo === void 0 ? void 0 : logo.id) !== null && _a !== void 0 ? _a : null,
            website: p.website,
            thu_tu: p.thu_tu,
        });
        if (result)
            console.log(`  ✓ Created partner: ${p.ten}`);
    }
}
// ══════════════════════════════════════════════
...
async function seedKhachHang() {
    ...
    const testimonials = [
        {
            ...
            noi_dung: "Timeso đã giúp chúng tôi tiết kiệm rất nhiều thời gian trong việc quản lý nhân sự. Từ việc chấm công, sắp ca đến tính lương đều được tự động hóa, giảm thiểu sai sót đáng kể.",
        ...
        },
        {
            ...
            noi_dung: "Trước khi sử dụng Timeso, việc quản lý 30 nhân viên là cả một vấn đề. Giờ đây mọi thứ đều trơn tru, từ việc xếp ca đến theo dõi hiệu suất.",
        ...
        },
        {
            ...
            noi_dung: "Tính năng AI của Timeso thực sự ấn tượng. Hệ thống tự động đề xuất lịch ca phù hợp, giúp chúng tôi tối ưu chi phí nhân sự đến 30%.",
            avatar_path: "images/about/d6604c8227042bb19de4e5724ae83e5aa40c0cba.png",
    ...
    for (const t of testimonials) {
        ...
        const result = await createCollectionEntry("khach-hangs", {
            ...
            noi_dung: t.noi_dung,
            avatar: (_a = avatar === null || avatar === void 0 ? void 0 : avatar.id) !== null && _a !== void 0 ? _a : null,
            logo_cong_ty: (_b = logo === null || logo === void 0 ? void 0 : logo.id) !== null && _b !== void 0 ? _b : null,
            so_sao: t.so_sao,
        });
        if (result)
            console.log(`  ✓ Created testimonial: ${t.ten}`);
    }
}
// ══════════════════════════════════════════════
    ...
    const articles = [
        {
            ...
            mo_ta: "Khám phá 5 xu hướng quản lý nhân sự hàng đầu trong năm 2025 giúp tối ưu hiệu quả và nâng cao trải nghiệm nhân viên.",
...
Tích hợp kênh liên lạc nội bộ trong cùng một nền tảng giúp tăng hiệu quả giao tiếp và giảm chi phí sử dụng nhiều công cụ.`,
        ...
        },
        {
            ...
            mo_ta: "Phương pháp giúp chuỗi cửa hàng F&B giảm 20-30% chi phí nhân sự mà vẫn đảm bảo chất lượng phục vụ.",
...
Áp dụng các ca ngắn 4-6 tiếng thay vì ca 8 tiếng cố định, phù hợp với nhu cầu thực tế.`,
        ...
        },
        {
            ...
            mo_ta: "Phân tích vai trò của trí tuệ nhân tạo trong lĩnh vực quản lý nhân sự và những tác động đến doanh nghiệp.",
...
AI phân tích patterns và tự đề xuất lịch ca tối ưu, cân bằng giữa nhu cầu kinh doanh và mong muốn của nhân viên.`,
    ...
    for (const art of articles) {
        ...
        const result = await createCollectionEntry("bai-viets", {
            ...
            tags: art.tags,
            noi_bat: art.noi_bat,
            hinh_dai_dien: (_a = image === null || image === void 0 ? void 0 : image.id) !== null && _a !== void 0 ? _a : null,
        });
        if (result)
            console.log(`  ✓ Created article: ${art.tieu_de}`);
    }
}
// ══════════════════════════════════════════════
    ...
    const caseStudies = [
        {
            ...
            mo_ta: "Câu chuyện The Coffee House tối ưu quy trình quản lý hơn 500 nhân viên tại 50+ cửa hàng với Timeso.",
            noi_dung: `## Thách thức
...
- **0 phút** mỗi tuần cho việc xếp lịch thủ công`,
    ...
    for (const cs of caseStudies) {
        ...
        if (result)
            console.log(`  ✓ Created case study: ${cs.tieu_de}`);
    }
}
// ══════════════════════════════════════════════
// 6. VỀ CHÚNG TÔI - Single Type
// ══════════════════════════════════════════════
async function seedVeChungToi(memberIds) {
    var _a, _b, _c, _d, _e;
    console.log("\n━━━ Seeding Về Chúng Tôi ━━━\n");
    const storyImage = await uploadFile("images/about/73b7c2a56de9b2bde5d83664188677534e98ec81.png", "Câu chuyện Timeso");
    // Upload core value icons
    const icon1 = await uploadFile("images/why-choose/35df67a1bba1a53f805dac9cf1055580f424e36e.svg", "Sáng tạo");
    const icon2 = await uploadFile("images/why-choose/706259e242d261fda917b658bedd7f2bb5b912e9.svg", "Chất lượng");
    const icon3 = await uploadFile("images/why-choose/550bffc06cf3f26dfdea9ef3b6ab7833848daabe.svg", "Tận tâm");
    // Upload CTA image
    const ctaImage = await uploadFile("images/cta/927772feb12cf7bb3461f8731b250063c9238795.png", "Timeso CTA");
    await updateSingleType("ve-chung-toi", {
        seo: {
            tieu_de: "Về Chúng Tôi - Timeso",
            mo_ta: "Tìm hiểu về Timeso - đội ngũ phát triển giải pháp quản lý nhân sự thông minh hàng đầu Việt Nam.",
            tu_khoa: "Timeso, quản lý nhân sự, về chúng tôi, đội ngũ",
        },
        hero: {
            tieu_de: "Timeso – Giải pháp vượt qua mọi",
            mo_ta: "Timeso cung cấp giải pháp quản lý nhân sự và vận hành thông minh cho các cửa hàng, giúp tối ưu quy trình, tiết kiệm thời gian và phát triển bền vững.",
...
Chính từ nhu cầu đó, đội ngũ sáng lập Timeso quyết tâm tạo nên một nền tảng thông minh, dễ sử dụng nhưng đủ mạnh để tự động hóa những công việc phức tạp. Chúng tôi tin rằng khi cửa hàng tiết kiệm được thời gian, họ sẽ có nhiều hơn để tập trung vào điều quan trọng nhất: phát triển con người và tạo ra giá trị thật sự.`,
        hinh_cau_chuyen: (_a = storyImage === null || storyImage === void 0 ? void 0 : storyImage.id) !== null && _a !== void 0 ? _a : null,
        gia_tri_cot_loi: [
            {
                tieu_de: "Sáng Tạo Không Ngừng",
                mo_ta: "Chúng tôi liên tục nghiên cứu và phát triển các giải pháp mới, ứng dụng công nghệ tiên tiến nhất để mang đến sản phẩm tốt nhất cho khách hàng.",
                icon: (_b = icon1 === null || icon1 === void 0 ? void 0 : icon1.id) !== null && _b !== void 0 ? _b : null,
            },
            {
                tieu_de: "Chất Lượng Hàng Đầu",
                mo_ta: "Mỗi sản phẩm của Timeso đều trải qua quá trình kiểm thử nghiêm ngặt, đảm bảo chất lượng và độ tin cậy cao nhất cho doanh nghiệp.",
                icon: (_c = icon2 === null || icon2 === void 0 ? void 0 : icon2.id) !== null && _c !== void 0 ? _c : null,
            },
            {
                tieu_de: "Tận Tâm Phục Vụ",
                mo_ta: "Đội ngũ hỗ trợ Timeso luôn sẵn sàng đồng hành cùng khách hàng 24/7, từ triển khai đến vận hành, giải quyết mọi vấn đề nhanh chóng.",
                icon: (_d = icon3 === null || icon3 === void 0 ? void 0 : icon3.id) !== null && _d !== void 0 ? _d : null,
            },
        ],
        doi_ngu: memberIds,
        cta: {
            tieu_de: "Timeso giúp chủ quán nhỏ vươn tới thành công lớn!",
            mo_ta: "Từng trải qua thiếu nhân sự, xoay ca gấp và sai lệch lương thưởng, chúng tôi tạo ra Timeso để giúp quản lý nhân sự, chia ca và tuyển dụng trở nên dễ dàng, minh bạch và hiệu quả hơn cho mọi cửa hàng.",
            hinh_nen: (_e = ctaImage === null || ctaImage === void 0 ? void 0 : ctaImage.id) !== null && _e !== void 0 ? _e : null,
            app_store_url: "https://apps.apple.com/app/timeso",
            google_play_url: "https://play.google.com/store/apps/details?id=com.timeso",
        },
    });
...
async function seedLienHe() {
    ...
    await updateSingleType("lien-he", {
        seo: {
            ...
            mo_ta: "Liên hệ với Timeso để được tư vấn giải pháp quản lý nhân sự phù hợp cho doanh nghiệp của bạn.",
            tu_khoa: "liên hệ, Timeso, tư vấn, hỗ trợ",
        },
        hero: {
            tieu_de: "Liên hệ với chúng tôi",
            mo_ta: "Hãy liên hệ với đội ngũ Timeso để được tư vấn giải pháp phù hợp nhất cho doanh nghiệp của bạn.",
        },
...
async function seedTrangCaseStudy() {
    ...
    await updateSingleType("trang-case-study", {
        seo: {
            ...
            mo_ta: "Khám phá các case study về việc triển khai Timeso tại các doanh nghiệp. Xem kết quả thực tế từ các khách hàng.",
...
async function seedTrangBlog() {
    ...
    await updateSingleType("trang-blog", {
        seo: {
            tieu_de: "Blog - Timeso",
            mo_ta: "Tin tức, hướng dẫn và insights về quản lý nhân sự, chấm công, và các giải pháp HR thông minh từ Timeso.",
            tu_khoa: "blog, tin tức, quản lý nhân sự, HR, Timeso",
        },
        hero: {
            tieu_de: "Blog & Tin Tức",
            mo_ta: "Cập nhật những kiến thức mới nhất về quản lý nhân sự, công nghệ HR và các giải pháp kinh doanh thông minh.",
        },
